...
void initialize_props(netvar_table_map& table_map)
{
    ...
    for (auto client_class = g_CHLClient->GetAllClasses(); client_class; client_class = client_class->m_pNext)
    {
...
uintptr_t netvarsystem::get_net_var(const uint32_t table, const uint32_t prop)
{
