...
bool ImGui::ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags)
{
    ...
    if ((flags & ImGuiColorEditFlags_DisplayHSV) && (flags & ImGuiColorEditFlags_DisplayRGB))
        ColorConvertHSVtoRGB(f[0], f[1], f[2], f[0], f[1], f[2]);
    else if ((flags & ImGuiColorEditFlags_DisplayRGB) && (flags & ImGuiColorEditFlags_DisplayHSV))
    ...
    if (label != label_display_end && !(flags & ImGuiColorEditFlags_NoLabel))
    {
        ...
        PushStyleColor(ImGuiCol_Text, ImColor(255, 255, 255, 255).operator ImVec4());
    ...
    if (!(flags & ImGuiColorEditFlags_NoSmallPreview))
    {
        ...
        if (BeginPopup("picker"))
        {
            ...
            ImGuiColorEditFlags picker_flags_to_forward = ImGuiColorEditFlags_DataTypeMask_ | ImGuiColorEditFlags_PickerMask_ | ImGuiColorEditFlags_DisplayMask_ | ImGuiColorEditFlags_HDR | ImGuiColorEditFlags_NoAlpha | ImGuiColorEditFlags_AlphaBar;
            ...
            PushItemWidth(square_sz * 8.0f); // Use 256 + bar sizes?
    ...
    if (value_changed && picker_active_window == NULL)
    {
        ...
        if ((flags & ImGuiColorEditFlags_DisplayHSV) && (flags & ImGuiColorEditFlags_DisplayRGB))
            ColorConvertHSVtoRGB(f[0], f[1], f[2], f[0], f[1], f[2]);
        if ((flags & ImGuiColorEditFlags_DisplayRGB) && (flags & ImGuiColorEditFlags_DisplayHSV))
            ColorConvertRGBtoHSV(f[0], f[1], f[2], f[0], f[1], f[2]);
    ...
    if ((g.LastItemData.StatusFlags & ImGuiItemStatusFlags_HoveredRect) && !(flags & ImGuiColorEditFlags_NoDragDrop) && BeginDragDropTarget())
    {
        ...
        if (accepted_drag_drop && (flags & ImGuiColorEditFlags_DisplayHSV))
...
bool ImGui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
{
    ...
    if (flags & ImGuiColorEditFlags_InputRGB)
    {
        ...
        if (memcmp(g.ColorEditSavedColor, col, sizeof(float) * 3) == 0)
        {
            if (S == 0)
    ...
    if (!(flags & ImGuiColorEditFlags_NoSidePreview))
    {
        ...
        static float color_save[4];
        if (Button("copy", ImVec2(40, 32)))
        {
            color_copy[0] = col[0];
            color_copy[1] = col[1];
            color_copy[2] = col[2];
        ...
        Spacing();
        ImVec4 cur_col_v4(col[0], col[1], col[2], (flags & ImGuiColorEditFlags_NoAlpha) ? 1.0f : col[3]);
        ColorButton("current", cur_col_v4, (flags & ImGuiColorEditFlags_HDR | ImGuiColorEditFlags_AlphaPreview | ImGuiColorEditFlags_AlphaPreviewHalf | ImGuiColorEditFlags_NoTooltip | ImGuiColorEditFlags_NoInputs | ImGuiColorEditFlags_NoLabel), ImVec2(32, 16));
        Spacing();
        if (ref_col != NULL)
        {
        ...
        ImVec4 save_col_v4(color_save[0], color_save[1], color_save[2], (flags & ImGuiColorEditFlags_NoAlpha) ? 1.0f : color_save[3]);
        color_save[0], color_save[1], color_save[2], color_save[3] = save_col_v4.x, save_col_v4.y, save_col_v4.z, save_col_v4.w;
        Spacing();
        if (ColorButton("save color", save_col_v4, (flags & (ImGuiColorEditFlags_HDR | ImGuiColorEditFlags_AlphaPreview | ImGuiColorEditFlags_NoTooltip | ImGuiColorEditFlags_NoInputs | ImGuiColorEditFlags_NoLabel | ImGuiColorEditFlags_NoInputs | ImGuiColorEditFlags_NoLabel)), ImVec2(32, 16)))
        {
            memcpy(col, color_save, components * sizeof(float));
            value_changed = true;
        }
        PopItemFlag();
        EndGroup();
    ...
    if (value_changed_h || value_changed_sv)
    {
        if (flags & ImGuiColorEditFlags_InputRGB)
        {
            ...
            memcpy(g.ColorEditSavedColor, col, sizeof(float) * 3);
        }
        else if (flags & ImGuiColorEditFlags_InputHSV)
        {
            col[0] = H;
    ...
    if (value_changed)
    {
        if (flags & ImGuiColorEditFlags_InputRGB)
        {
            ...
            B = col[2];
            ColorConvertRGBtoHSV(R, G, B, H, S, V);
            if (memcmp(g.ColorEditSavedColor, col, sizeof(float) * 3) == 0) // Fix local Hue as display below will use it immediately.
            {
                if (S == 0)
                    H = g.ColorEditSavedHue;
                if (V == 0)
    ...
    draw_list->AddRectFilled(sv_cursor_pos - ImVec2(1, 1), sv_cursor_pos + ImVec2(1, 1), ImColor(col[0], col[1], col[2], col[3]));

    draw_list->AddRectFilled(sv_cursor_pos - ImVec2(9, 2), sv_cursor_pos + ImVec2(-3, 2), ImColor(0, 0, 0));
    draw_list->AddRectFilled(sv_cursor_pos - ImVec2(8, 1), sv_cursor_pos + ImVec2(-4, 1), ImColor(col[0], col[1], col[2], col[3]));

    draw_list->AddRectFilled(sv_cursor_pos + ImVec2(3, -2), sv_cursor_pos + ImVec2(9, 2), ImColor(0, 0, 0));
    draw_list->AddRectFilled(sv_cursor_pos + ImVec2(4, -1), sv_cursor_pos + ImVec2(8, 1), ImColor(col[0], col[1], col[2], col[3]));

    draw_list->AddRectFilled(sv_cursor_pos - ImVec2(2, 9), sv_cursor_pos + ImVec2(2, -3), ImColor(0, 0, 0));
    draw_list->AddRectFilled(sv_cursor_pos - ImVec2(1, 8), sv_cursor_pos + ImVec2(1, -4), ImColor(col[0], col[1], col[2], col[3]));

    draw_list->AddRectFilled(sv_cursor_pos + ImVec2(-2, 3), sv_cursor_pos + ImVec2(2, 9), ImColor(0, 0, 0));
    draw_list->AddRectFilled(sv_cursor_pos + ImVec2(-1, 4), sv_cursor_pos + ImVec2(1, 8), ImColor(col[0], col[1], col[2], col[3]));
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    if (span_all_columns && window->DC.CurrentColumns)
        ...
        const ImU32 col = GetColorU32((held && hovered) ? ImGuiCol_Text : hovered ? ImGuiCol_Text : ImGuiCol_TextDisabled);
        PushStyleColor(ImGuiCol_Text, col);
